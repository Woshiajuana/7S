...
class _HistoryListViewState extends State<HistoryListView> {
  ...
  List<HistoryJsonModel> _arrData;
  int _numIndex = 1;
  int _numSize = 10;
  ...
  Widget _widgetPhotoCellItem (index) {
    HistoryJsonModel historyJsonModel = _arrData[index];
    PhotoJsonModel photoJsonModel = historyJsonModel.photo;
    String strTime = historyJsonModel.created_at != null
        ? new DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.parse(historyJsonModel.created_at).toLocal())
        : '';
  ...
  void _reqHistoryList ({
    ...
    await Future.delayed(Duration(milliseconds: 0)).then((e) async{
      try {
        String strUrl = Application.config.api.reqHistoryList;
        ...
        setState(() {
          List<HistoryJsonModel> data = _listJsonModel.list.map((item) => HistoryJsonModel.fromJson(item)).toList();
